...
const MyMarket = () => {
  ...
  const loadInitialData = async () => {
    try {
      ...
      const productsResponse = await agriConnectAPI.market.getPosts(null, null, 'product', null, null, false, 1, 50);
      if (productsResponse.success) {
        // Filter to show only current user's products
        const userProducts = productsResponse.posts.filter(p => p.user_id == userId);
        setProducts(userProducts);
      }

      // Load user notifications
      try {
        const notificationsResponse = await agriConnectAPI.market.getUserNotifications();
        if (notificationsResponse.success) {
          setNotifications(notificationsResponse.notifications || []);
        }
      } catch (error) {
        // Notifications endpoint might not exist yet, use mock data
