...
import { FiSearch, FiFilter, FiPlus, FiEdit, FiTrash2, FiCheckCircle, FiXCircle, FiUpload, FiImage } from 'react-icons/fi';
...
function MyMarket() {
  ...
  const fetchMyPosts = async () => {
    try {
      ...
      setError('Failed to fetch your market posts');
  ...
  const handleUpdatePost = async (postId, updates) => {
    try {
      await agriConnectAPI.market.updatePost(postId, updates);
  ...
  if (loading) {
    return (
      <div className="flex justify-center items-center h-64">
        ...
        <span className="ml-3 text-gray-600">Loading your market posts...</span>
      </div>
    );
  }
      ...
      <header className="bg-white rounded-xl shadow-sm p-6 border border-gray-100">
        <div className="flex justify-between items-center mb-6">
          <div>
            <h1 className="text-2xl sm:text-3xl font-bold text-green-800">
              <span className="text-3xl">🛒</span> My Marketplace
            </h1>
            <p className="text-gray-600">Manage your listings and track sales</p>
          </div>
          <button 
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          ...
          <div className="bg-purple-50 p-4 rounded-lg border border-purple-100">
            <h3 className="text-sm font-medium text-purple-800">Potential Earnings</h3>
      ...
      <section className="bg-white rounded-xl shadow-sm p-6 border border-gray-100">
        <div className="flex justify-between items-center mb-6">
          <h2 className="text-xl sm:text-2xl font-bold text-green-700 flex items-center">
            <span className="text-2xl mr-2">📋</span> Your Active Listings
          </h2>
          <span className="text-sm text-gray-500 flex items-center">
            {filteredPosts.length} of {myPosts.length} listings
          </span>
        </div>
        ...
        {filteredPosts.length === 0 ? (
          <div className="text-center py-12">
            ...
            <h3 className="mt-2 text-lg font-medium text-gray-900">
              {showCreateForm ? 'Form submitted successfully!' : "You haven't created any listings yet"}
            </h3>
            <p className="mt-1 text-sm text-gray-500">
              {showCreateForm 
                ? 'Your listing will appear here once approved.' 
              ...
              return (
                <div key={post.id} className="bg-gray-50 border border-gray-200 rounded-lg p-6 hover:shadow-md transition-all">
                  ...
                  <div className="flex flex-wrap gap-2 pt-4 border-t border-gray-100">
                    <button
                      onClick={() => handleUpdatePost(post.id, { is_available: !post.is_available })}
